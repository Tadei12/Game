...
if language == "eng":
    ...
    Table_button = ShopButton((25, 150), "Buy a table - 150$")
    Floor_button = ShopButton((550, 150), "Floor upgrade - 400$")
    ...
    Table_button = ShopButton((25, 150), "Купити стіл - 150$")
    Floor_button = ShopButton((550, 150), "Нова підлога - 400$")
...
def day():
    ...
    if current_floor:
        window.blit(floor_better, (0, 0))
    else:
        window.blit(floor_default, (0, 0))
    
    window.blit(day_stage, (0, 0))
    
    window.blit(tables_list[current_tables-2], (0, 0))
...
def upgrades_shop():
    global current_floor, current_tables, money
    window.blit(shop_background, (0, 0))

    draw_text(f"{money}$", standart_font, (1000, 50))
    if language == "eng":
        draw_text("Interior", standart_font, (550, 100))
        draw_text("Repair mascots", standart_font, (550, 350))
    else:
        draw_text("Інтер'єр", standart_font, (550, 100))
        draw_text("Ремонт маскотів", standart_font, (550, 350))
...
def UpdateMascots():
    global mascot_timer
    if pygame.time.get_ticks() - mascot_timer > 8000:
        mascot_timer = pygame.time.get_ticks()
        BunnyMascot()
        FoxMascot()
        DogMascot()
...
def update_and_save():
    ...
    money += current_floor*25 + current_tables*50
...
def LoadFunc():
    global settings, money, current_floor, current_tables, current_day
    with open("assets/dat.json", "r") as file:
        settings = json.load(file)
